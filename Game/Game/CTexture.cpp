...
D3DXMATRIX CTexture::m_MatCamera;

void CTexture::UpdateCamera(D3DXMATRIX matCamera)
{
...
void CTexture::UpdateMat(RenderInfo* pGameInfo)	//행렬 정보 업데이트
{
	...
	m_matWorld =m_MatCamera* m_matWorld;
...
void CTexture::Draw(CSprite* pSprite,int frame, RenderInfo* pGameInfo)	//차후바꿀것
{
	...
	RECT rcSrc = pSprite->GetFrameRect(frame);
	D3DXVECTOR3 vecCenter = pSprite->GetFrameCenter();
...
CSprite* CTexture::GetSprite(wstring sprite)
{
