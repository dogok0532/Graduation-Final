...
void CPlayerComponent::Update(float deltaTime)
{
	if (m_pCharacterComponent)
	{
		...
		if (GetAsyncKeyState(VK_LBUTTON))
		{
			...
			{	
				m_pCharacterComponent->CreateBullet(false);
...
void CPlayerComponent::SetWay()
{
	...
	vecMouse.x = MousePos.x  + CCamera::GetInstance()->GetPos().x;
	vecMouse.y = MousePos.y + CCamera::GetInstance()->GetPos().y;
	vecMouse.z = 0;
	...
	if (m_pCharacterComponent)
	{
		m_pCharacterComponent->SetAngle(fDegree);
	}
